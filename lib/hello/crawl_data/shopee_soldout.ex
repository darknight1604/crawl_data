defmodule ShopeeSoldOut do
    ...
    def readEachData(item) do
        contain_data_index = Tuple.to_list(item)
        id = Enum.at(contain_data_index, 1)
        # # children = <a>
        childrens = ReadHtml.extractHtml(Tuple.to_list(Enum.at(contain_data_index, 0)))
        ...
        if(image_src != nil) do
            ...
            %{product_name: String.trim(product_name), price_group: price_group, image_product: image_product, id: id}
        else
            # response
            IO.puts "image product miss at product: #{product_name}"
            %{product_name: String.trim(product_name), price_group: price_group, image_product: "", id: id}
        end
