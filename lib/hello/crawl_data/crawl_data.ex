...
defmodule CrawlData do
    use Hound.Helpers

    def run(type_crawl) do
        Hound.start_session
        ...
        result =
            case type_crawl do
                0 -> crawlCollection(document)
                1 -> crawlSearchView(document)
                2 -> crawlSoldOut(document)
            end
    ...
    def crawlCollection(document) do
        ...
        if status === :ok do
            ...
            if list_item_collection_src != nil do
                ...
                ShopeeCollection.run(Enum.with_index(list_item_collection))
            else
    ...
    def crawlSearchView(document) do
        ...
        if item_search_view_group_src != nil do
            ...
            ShopeeSearchView.run(Enum.with_index(list_item_search))
        else
            IO.puts "error find div.shop-search-result-view"
        end
    end

    def crawlSoldOut(document) do
        ...
        if status === :ok do
            ...
            ShopeeSoldOut.run(Enum.with_index(list_item_sold_out))
